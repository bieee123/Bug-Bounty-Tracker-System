...
// Delete comments - PERBAIKAN KODE UNTUK DELETE COMMENT
if (isset($_POST['delete_comment']) && isset($_POST['comment_id'])) {
    $comment_id = (int)$_POST['comment_id'];
    
    // Verifikasi bahwa comment milik user ini atau user adalah admin
    ...
    if ($check_stmt) {
        ...
        if ($comment_data = $check_result->fetch_assoc()) {
            // Jika user adalah pemilik komentar atau admin
            if ($comment_data['user_id'] == $user_id || is_admin()) {
                ...
                if ($delete_stmt) {
                    ...
                    if ($delete_stmt->execute()) {
                        ...
                        $error = "Failed to delete comment: " . $conn->error;
                    }
                    $delete_stmt->close();
                } else {
        ...
        $error = "Error checking comment ownership: " . $conn->error;
    }
}
